...
def faturamento2025NE(dados:pd.DataFrame) -> pd.DataFrame:
    ...
    dados['Soma_NE_temp'] = np.where(((dados['DISTRIBUIDOR'] == 'GRUPO_NORDESTE') & (dados['UF'].isin(['CE','PI','MA']))), dados['Soma ult.6 meses'], 0)
    
    dados['Performance (2025) - NE'] = dados.groupby(['UF','PRODUTO','TIPO_FARMACIA'])['Soma_NE_temp'].transform('sum')
